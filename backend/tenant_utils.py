...
def get_tenant_record_or_403(request: Request, db: Session = Depends(get_db)) -> Tenant:
    """Get tenant record or return 403 if not found"""
    tenant_slug = request.headers.get('X-Tenant')
    if not tenant_slug:
        raise HTTPException(status_code=403, detail='Missing X-Tenant header')
    tenant = db.query(Tenant).filter_by(slug=tenant_slug).first()
    if not tenant:
