...
from models import Referrals, Users
...
def generate_referral_code(db: Session = Depends(get_db), user: Users = Depends(get_current_user)):
    """Generate a unique referral code for the current user."""
    existing = db.query(Referrals).filter_by(user_id=user.id).first()
    if existing:
        return existing
    code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))
    referral = Referrals(user_id=user.id, code=code, rewards={"leads": 50, "credits": 100})
    db.add(referral)
    db.commit()
    db.refresh(referral)
    return referral
...
def apply_referral_code(req: ApplyReferralRequest, db: Session = Depends(get_db), user: Users = Depends(get_current_user)):
    """Apply a referral code as a new user to receive rewards."""
    referral = db.query(Referrals).filter_by(code=req.code).first()
    if not referral:
...
def get_referral_status(db: Session = Depends(get_db), user: Users = Depends(get_current_user)):
    """Get all referral codes generated by the current user."""
    codes = db.query(Referrals).filter_by(user_id=user.id).all()
    return codes 