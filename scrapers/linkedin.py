...
class LinkedInScraper(BaseScraper):
    ...
    def __init__(self, per_request_delay_seconds: float = 2.5, max_fetch_bytes: int = 1024 * 1024) -> None:
        self.per_request_delay_seconds = per_request_delay_seconds
        self.client = HttpClient()
        self.max_fetch_bytes = max_fetch_bytes

    def _extract_company(self, url: str, query: str) -> ScrapeResult:
        ...
        followers = ""
        m = re.search(r"([\d,\.]+)\s+followers", desc, flags=re.I)
        if m:
            followers = m.group(1)
    ...
    def search(self, query: str, max_results: int) -> Iterable[ScrapeResult]:
        ...
        candidates: List[str] = site_search(query=query, site="linkedin.com/company", num=search_limit, engine="duckduckgo", debug=False)
        if not candidates:
            return
        
        for url in candidates:
            try:
                result = self._extract_company(url, query)
